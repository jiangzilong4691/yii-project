...
class FTP extends Filesystem
{
     ...
     * $count = $I->grabFileList('TEST', false); // Include . .. .thumbs.dbConnection
     * ?>
     * ```
     *
     * @param string $path
     * @param bool $ignore - suppress '.', '..' and '.thumbs.dbConnection'
     * @return array
     */
    public function grabFileList($path = '', $ignore = true)
    {
        ...
        if (is_array($files) && !empty($files)) {
            ...
            foreach ($files as &$file) {
                if (strtolower($file) != '.' &&
                    strtolower($file) != '..' &&
                    strtolower($file) != 'thumbs.dbConnection'
                ) { // Ignore '.', '..' and 'thumbs.dbConnection'
     ...
     * $count = $I->grabFileCount('TEST', false); // Include . .. .thumbs.dbConnection
     * ?>
     * ```
     *
     * @param string $path
