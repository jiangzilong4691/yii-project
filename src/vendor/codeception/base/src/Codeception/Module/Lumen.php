...
class Lumen extends Framework implements ActiveRecord, PartedModule
{
    ...
    public function _before(TestInterface $test)
    {
        ...
        if ($this->app['dbConnection'] && $this->config['cleanup']) {
            $this->app['dbConnection']->beginTransaction();
        }
    }
    ...
    {
        if ($this->app['dbConnection'] && $this->config['cleanup']) {
            $this->app['dbConnection']->rollback();
        }

        // disconnect from DB to prevent "Too many connections" issue
        if ($this->app['dbConnection']) {
            $this->app['dbConnection']->disconnect();
        }
    }
    ...
    public function haveRecord($table, $attributes = [])
    {
        ...
        try {
            return $this->app['dbConnection']->table($table)->insertGetId($attributes);
        } catch (\Exception $e) {
    ...
    {
        $query = $this->app['dbConnection']->table($table);
        foreach ($attributes as $key => $value) {
            $query->where($key, $value);
        }
