...
class Phalcon extends Framework implements ActiveRecord, PartedModule
{
    ...
    public function _before(TestInterface $test)
    {
        ...
        if ($this->config['cleanup'] && $this->di->has('dbConnection')) {
            if ($this->config['savepoints']) {
                $this->di['dbConnection']->setNestedTransactionsWithSavepoints(true);
            }
            $this->di['dbConnection']->begin();
        ...
        $this->client->setApplication(function () use ($bootstrap) {
            ...
            if ($currentDi->has('dbConnection')) {
                $di['dbConnection'] = $currentDi['dbConnection'];
            }
    ...
    {
        if ($this->config['cleanup'] && isset($this->di['dbConnection'])) {
            while ($this->di['dbConnection']->isUnderTransaction()) {
                $level = $this->di['dbConnection']->getTransactionLevel();
                try {
                    $this->di['dbConnection']->rollback(true);
                    $this->debugSection('Database', 'Transaction cancelled; all changes reverted.');
                } catch (PDOException $e) {
                }
                if ($level == $this->di['dbConnection']->getTransactionLevel()) {
                    break;
                }
            }
            $this->di['dbConnection']->close();
        }
        $this->di = null;
        Di::reset();
