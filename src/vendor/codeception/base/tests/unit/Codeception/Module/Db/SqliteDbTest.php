...
class SqliteDbTest extends TestsForDb
{
    public function getPopulator()
    {
        ...
        @chmod('tests/data/sqlite.dbConnection', 0777);
        return 'cat '. $config['dump'] .' | sqlite3 tests/data/sqlite.dbConnection';
    }
    ...
    public function getConfig()
    {
        ...
        return [
            'dsn' => 'sqlite:tests/data/sqlite.dbConnection',
            'user' => 'root',
            'password' => '',
    ...
    public function testMultiDatabase()
    {
        $config = array_merge($this->getConfig(), [
            'dsn' => 'sqlite:tests/data/sqlite1.dbConnection',
            'cleanup' => false
        ]);
    ...
    {
        $config = array_merge($this->getConfig(), ['dsn' => 'sqlite:tests/data/sqlite1.dbConnection']);
        $this->module->_reconfigure(
            [
                'cleanup' => false,
                'databases'   => ['db2' => $config],
